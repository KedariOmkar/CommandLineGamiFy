        ...
        while True:
            ...
            if selected_option == 1:
                ...
                def insert_data(skill_table, value):
                    ...
                    find_max_field_id = f"SELECT MAX(id) FROM {skill_table} WHERE field = '{value}'"
                ...
                while True:
                    ...
                    if selected_option == 1:
                        while True:
                            ...
                            starting_count_dict = {"atma": 100, "brain": 200, "body": 300}
                            ...
                            while True:
                                ...
                                print("")
                                another_skill = input(" ! Again (0 to go back): ").lower()
                                if another_skill == '0':
                                    break  # Go back to the previous screen
                    ...
                    if selected_option == 2:
                        while True:
                            ...
                            starting_count_dict = {"lang": 100, "dsa": 200, "sub": 300, "tech": 400, "tools": 500,
                                                   "build": 600}
                            field_dict = {1: "lang", 2: "dsa", 3: "sub", 4: "tech", 5: "tools", 6: "build"}
                    ...
                    if selected_option == 4:
                        # Store menu options
                        store_options = {
                            1: {"category": "PROFESSIONAL", "fields": ["position", "tags"]},
                            2: {"category": "FINANCIAL", "fields": ["companies", "investments", "liquid"]},
                            3: {"category": "LAVISH", "fields": ["mansions", "cars", "ultra", "lifestyle", "hoe"]},
                            4: {"category": "PERSONAL", "fields": ["self", "parents","relations"]},
                            5: {"category": "PHILANTHROPIC", "fields": ["Food","Education","Climate Change"]},
                        }


                        # Function to insert data into MySQL
                        def insert_into_mysql(ob_name , ob_cat , on_sub_cat , ob_field , ob_price , ob_achieved_date , ob_age , status):
                            sql.server.execute(f"insert into objectives (ob_name, ob_cat, ob_sub_cat, ob_field, ob_price, ob_achieved_date, ob_age, status)  values ('{ob_name}','{ob_cat}','{on_sub_cat}','{ob_field}',{ob_price},'{ob_achieved_date}','{ob_age}','{status}')")
                            sql.engine.commit()


                        # Function to update existing entry in MySQL
                        def update_existing_mysql(ob_name , ob_cat , on_sub_cat , ob_field , ob_price , ob_achieved_date , ob_age , status):
                            ...
                            values = (ob_name , ob_cat , on_sub_cat , ob_field , ob_price , ob_achieved_date , ob_age , status)
                        ...
                        def input_data_and_pics():
                            ...
                            if "fields" in store_options[select]:
                                ...
                                print(
                                    "------------------------------------------------------------------------------------------------------------")
                                for i, field in enumerate(store_options[select]["fields"], start=1):
                                    print(f"\t{i}. {field.upper()}")
                                print(
                                    "------------------------------------------------------------------------------------------------------------")

                                field_index = int(input("Select Field: ")) - 1
                                if field_index < 0 or field_index >= len(store_options[select]["fields"]):
                            ...
                            if name_exists_in_mysql(ob_name):
                                ...
                                update_option = input("Do you want to update the existing entry? (yes/no): ").lower()
                                if update_option == 'no':
                                    return
                            ...
                            if name_exists_in_mysql(ob_name):
                                ...
                                if update_existing_option == '1':
                                    update_existing_mysql(ob_name , ob_cat , ob_sub_cat , ob_field , ob_price , ob_achieved_date , ob_age , status)
                                else:
                                    return
                            else:
                                insert_into_mysql(ob_name , ob_cat , ob_sub_cat , ob_field , ob_price , ob_achieved_date , ob_age , status)
                        ...
                        while True:
                            ...
                            print("\t :: STORE")
                            print("------------------------------------------------------------------------------------------------------------")
                            print("\t\t 1.PROFESSIONAL   2.FINANCIAL   3.LAVISH   4.PERSONAL   5.PHILANTHROPIC")
                            print("------------------------------------------------------------------------------------------------------------")
            ...
            if selected_option == 2:
                ...
                def reset_total_tasks():
                    sql.server.execute('update report_tasks_total set total_tasks_done = 0')
                    sql.engine.commit()

                    sql.server.execute('update report_tasks_total set total_tasks_created = 0')
                    sql.engine.commit()
                ...
                def restart():
                    ...
                    if ask_sure == 'y':
                        ...
                        if ask_again == 'onk':
                            ...
                            report_tables = ['report_time_learn', 'report_time_recall', 'report_time_test',
                                             'report_mission_learn','report_mission_recall', 'report_mission_test',
                                             'character_physiqe_shield','character_physiqe_sharp','character_physiqe_silent','character_routine',
                                             'game_matrix_agents', 'report_session','report_money','game_bank_transaction','character_habit']

                            skill_tables = ['skill_sh', 'skill_tg', 'skill_en']
                            skill_fields = ['time_learn', 'time_recall', 'time_test',
                                            'mission_learn','mission_recall', 'mission_test']
    ...
    if selected_option == 2:
        def update_frame():
            while True:
                ...
                field_dict = {1: 'lang', 2: 'dsa', 3: 'sub', 4: 'tech', 5: 'tools', 6: 'build', 7: 'softs', 8: 'busi',
                              9: 'atma', 10: 'brain', 11: 'body'}

                report_dict_time = {1: 'report_time_learn', 2: 'report_time_recall', 3: 'report_time_recall'}

                report_dict_mission = {1: 'report_mission_learn', 2: 'report_mission_recall', 3: 'report_mission_test'}
            ...
            while True:
                ...
                if take_input == 1:
                    ...
                    def insert_data_into_game_competition(total_value,value):
                        sql.server.execute(f"update game_competition set {total_value} = {value} where cur_date = CURDATE()")
                        sql.engine.commit()
                    ...
                    total_money = int((random_time+random_mission+random_routine+random_tasks+random_kill)*10000000)
    ...
    if selected_option == 4:
        def pomodoro():
            ...
            def start_pomodoro():
                ...
                winsound.PlaySound("C:\\Users\\kedar\\Downloads\\marvel.wav", winsound.SND_FILENAME)
                print("\t ! Work Finished..")
                print("----")
                print("\t\t Break Time....")
                time.sleep(break_time_in_minutes * 60)
                winsound.PlaySound("C:\\Users\\kedar\\Downloads\\gta.wav", winsound.SND_FILENAME)
                print("\t ! Break ended..")
                start_pomodoro()

            if __name__ == "__main__":
                time_in_minutes, break_time_in_minutes = set_timer()
                start_prompt = input("Ready to start Pomodoro session? (yes/no): ").lower()
                if start_prompt == "yes":
                    start_pomodoro()
                else:
